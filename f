...
local fovCircle -- Variable to hold the FOV circle
...
-- Calculate dynamic FOV based on X, Y, Z
...
-- Check if the body part is within the FOV radius
local function isWithinFov(position)
    local mouseX, mouseY = getMousePosition()
    local screenPos = Camera:WorldToViewportPoint(position)
    local dynamicFovSize = calculateFov(shared.Forbidden.TriggerBot.FovSettings.X, shared.Forbidden.TriggerBot.FovSettings.Y, shared.Forbidden.TriggerBot.FovSettings.Z)
    local distance = (Vector2.new(screenPos.X, screenPos.Y) - Vector2.new(mouseX, mouseY)).Magnitude
    return distance <= dynamicFovSize
end

-- Get body parts of a character
...
-- Sync mouse with target position
...
-- Check if we are holding a knife
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    ...
                    if onScreen and isWithinFov(part.Position) then
...
-- Draw the FOV circle (invisible)
local function drawInvisibleFov()
    local dynamicFovSize = calculateFov(shared.Forbidden.TriggerBot.FovSettings.X, shared.Forbidden.TriggerBot.FovSettings.Y, shared.Forbidden.TriggerBot.FovSettings.Z)
    if not fovCircle then
        fovCircle = Drawing.new("Circle")
        fovCircle.Thickness = 2
        fovCircle.Transparency = 1  -- Make it invisible
        fovCircle.Filled = true
        fovCircle.Color = Color3.fromRGB(255, 0, 0)  -- Red color
    end

    local mouseX, mouseY = getMousePosition()
    fovCircle.Position = Vector2.new(mouseX, mouseY)
    fovCircle.Radius = dynamicFovSize
end

-- Input events for toggling the TriggerBot
...
-- Main loop to trigger the bot action and draw the FOV circle
RunService.RenderStepped:Connect(function()
    if isToggled then
        TriggerBotAction()
        drawInvisibleFov()  -- Draw the invisible FOV circle
    end
end)
